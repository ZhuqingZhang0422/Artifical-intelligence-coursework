...
class TimeoutFunction:
    def __init__(self, function, maxSeconds):
        self.timeout = maxSeconds
        self.function = function

    def __call__(self, *args):
        result = [TimeoutFunctionException("function '%s' self.timeout %s seconds exceeded" % (self.function.__name__, self.timeout))]
        def new_func():
            try:
                result[0] = self.function(*args)
